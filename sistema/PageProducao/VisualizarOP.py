...
def app():
    ...
    df_manuseio = df_propostas.loc[(df_propostas['OPStatus'] == 'OP Impressa') &
                               (~df_propostas['Locale'].isin(['', 'Expedição', 'Saiu para entrega', 'Entregue']))].copy()
    ...
    st.title('Gerenciamento de OP')
    
    # Exibindo a contagem de OPs pendentes
    st.subheader(f"Quantidade de OP's em produção: {contagem_locale}")
    st.write('---')

    st.subheader("OP's em produção")
    # Garantindo que 'NumeroProposta' seja tratado como string
    df_agrupado['NumeroProposta'] = df_agrupado['NumeroProposta'].astype(str)
    # Exibindo apenas as colunas especificadas
    st.dataframe(df_agrupado[['Locale', 'Vendedor', 'Cliente', 'DataEntrega', 'NumeroProposta', 'OPStatus']], use_container_width=True)
    st.write("---")

    # Permitindo ao usuário selecionar um produto para alterar o status para 'OP Impressa'
    st.subheader("Selecione um produto para atualizar o status")
    ...
    if os.path.exists(diretorio_upload):
        ...
        selected_file = st.selectbox("Escolha uma OP para visualizar", files)
